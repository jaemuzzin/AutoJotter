...
public class AutoJotter {
	 ...
	 * arg9=italics(y/n), arg10=svg output (y/n)
	 *
	...
	public static void main(String[] args) throws Exception {
		if (args.length == 0) {
			...
			run(Double.parseDouble(args[0]), Double.parseDouble(args[1]), Integer.parseInt(args[2]), Integer.parseInt(args[3]), args[4], args[5], Integer.parseInt(args[6]), args[7].equals("Y"), args[8].equals("Y"), args[9].equals("Y"), System.out, System.err, new BufferedReader(new InputStreamReader(System.in)));
		}
	}

	public static void run(double totalWidth, double totalHeight, int cols, int rows, String upcommand, String downcommand, int feedrate, boolean rotated, boolean italics, boolean svgOutput, PrintStream out, PrintStream err, BufferedReader in) {
		// load glyphs from svg
		alphabet.put(".", new Glyph(new BufferedReader(new InputStreamReader(AutoJotter.class.getResourceAsStream("/glyphs/period.svg"))).lines().reduce("", (s1, s2) -> s1 + "\r\n" + s2), '.', upcommand, downcommand, svgOutput));
			...
			if (whiteSpace.contains(c) || textCursor == text.length() - 1) {
				...
				if (cols - (gridCursor % cols) >= wordBuffer.length()) {
					for (char el : wordBuffer.toString().toCharArray()) {
						...
						SimpleMatrix svgTrans = SimpleMatrix.identity(3);
						if (svgOutput) {
							svgTrans.set(0, 0, 25.4);
							svgTrans.set(1, 1, -25.4);
							svgTrans.set(1, 2, cellHeight * (rows+1) * 25.4);// translate to cell pos
						}
							...
							grid[gridCursor / cols][gridCursor % cols]//this grid position is
								   = new Glyph(
										 alphabet.get("" + el).getPaths().stream()
											    .map(path
													  -> //the alphabet letter
													  new Path(path.getPoints().stream().map(p -> svgTrans.mult(rotate.mult(trans.mult(italicsSkew.mult(scaleDown.mult(p))))))//process list of points, transform them
													  .collect(Collectors.toList()), upcommand, downcommand, svgOutput))
											    .collect(Collectors.toList()), el, upcommand, downcommand, svgOutput);
						}
						gridCursor++;
					}
		...
		if (!svgOutput) {
			...
			for (int i = 0; i < grid.length; i++) {
				for (int j = 0; j < grid[i].length; j++) {
                                    out.println("; Character "+grid[i][j].toString());
			...
			out.println(new BufferedReader(new InputStreamReader(AutoJotter.class.getResourceAsStream("/svg/header.txt"))).lines().reduce((s1, s2) -> s1 + "\n" + s2).get());
			...
			out.println(new BufferedReader(new InputStreamReader(AutoJotter.class.getResourceAsStream("/svg/footer.txt"))).lines().reduce((s1, s2) -> s1 + "\n" + s2).get());
